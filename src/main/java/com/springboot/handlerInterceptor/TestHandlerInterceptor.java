...
1、类  要实现Spring 的HandlerInterceptor 接口
2、类  继承实现了HandlerInterceptor 接口的类，例如 已经提供的实现了HandlerInterceptor 
	接口的抽象类HandlerInterceptorAdapter
  @author 刘彦军
 ！！！除了定义此类，还需要将自定义的拦截器，注册到WebMvcConfigurer中，拦截器才起作用
 	之前我们在xml中配置拦截路径等，springboot我们需要继承WebMvcConfigurerAdapter类
 	不过springBoot2.0以上 WebMvcConfigurerAdapter 方法过时，
 	有两种替代方案：
